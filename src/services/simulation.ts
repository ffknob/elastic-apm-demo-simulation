...
import { randomNumber, randomUserContext } from '../shared/util';
...
class Simulation {
  ...
  createUserContext(
  ...
  ): APMUserContext | SimulationUserContext {
    ...
    if (userContext) {
      ...
      return randomUserContext();
    }
  }

  async init(simulationRequest: SimulationRequestLocals, delayOnInit: boolean) {
    const apmService = ApmService.getInstance();
    const delayGenerator = new DelayGenerator();

    const userContext:
      | APMUserContext
      | SimulationUserContext = this.createUserContext(simulationRequest);
    ...
    if (delayOnInit) {
      await delayGenerator
        ...
        .then((delay: number) => apmService.setLabel('random-delay', delay));
    }
  }
  ...
  async generateCapturedError(simulationRequest: SimulationRequestLocals) {
    ...
    apmService.captureError(error.message!, {
      handled: false,
      custom: {
        fake: true,
  ...
  async generateComplexTransaction(simulationRequest: SimulationRequestLocals) {
    ...
    for (let i = 0; i < complexTransaction!.totalSubTransactions; i++) {
      ...
      for (let j = 0; j < complexTransaction!.totalSpans; j++) {
        let randomSpanTypeIndex: number = randomNumber(SPAN_TYPES.length);
